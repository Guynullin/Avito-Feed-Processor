...
from .format_desc import format_forged_rim_desc, generate_rims_card_specs
from collect_cards import get_image_dict_wm
from config import TITLE_FIX_LIST
from datetime import datetime, timedelta


def record_forged_rims(cards: list, ws: worksheet, url_dict: dict):
    """writes forged rim cards from the database to a worksheet.
    ...
    and ws[1][18].value == 'ContactPhone' and ws[1][31].value == 'RimBrand':  
        ...
        for index, row in enumerate(ws.rows):
            if row[1].value == 'Id' or index <= 199:
                continue
            if row[1].value != None:
                ...
                if not row[1].value in db_cards_id_list:
                    ...
                    for card in cards:
                        if card['id'] == row[1].value:
                            ...
                            rim_desc = format_forged_rim_desc(card=card, sv=False)
        ...
        if len(cards) > 0:
            ...
            for card in cards:
                ...
                if ws[index][1].value == None:
                    ...
                    ws[index][3].value = format_forged_rim_desc(card=card, sv=False)
